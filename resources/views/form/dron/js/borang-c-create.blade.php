<script type="text/javascript">
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenDronBorangCDatatable').DataTable().ajax.reload();
                                    });
                                }
                            },
                            fail: (response) => {
    ...
    function addGambardron() {
        var count = $('#gambar_count_dron').val();
        count = ++count;

        $('#gambar_count_dron').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambardron(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambardron(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporandron() {
        var count = $('#laporan_count_dron').val();
        count = ++count;

        $('#laporan_count_dron').val(count);

        $('#addLaporandron').append('<tr id="addLaporandron' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporandron(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporandron(argument) {
        $('#addLaporandron' + argument).remove();
    }
</script>