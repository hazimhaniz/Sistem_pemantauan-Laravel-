<form id="formDAccess_{{ $elemen_id }}" method="POST" action="{{ route('form.submitborangD') }}">
  @csrf
  <div class="row">

    <input type="hidden" name="borangDID" value="{{ $borangD->id }}" />
    <div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
      <input type="hidden" name="flag" id="flagD1" value="">
      <ul class="pager wizard no-style">
        @if($borangD)
        @hasanyrole('eo')
        @if($borangD->status_id == 600)
        <li class="submit">
          <button onclick="submitFormDAccess_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
...
<script>
  function submitFormDAccess_{{$elemen_id}}(){
    var form = $("#formDAccess_{{ $elemen_id }}");
    $.ajax({
