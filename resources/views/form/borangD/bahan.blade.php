<form id="formDBahan_{{ $elemen_id }}" method="POST" action="{{ route('form.submitborangD') }}">
  @csrf
  <div class="row">
    ...
    <div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
      <ul class="pager wizard no-style">
        ...
        <li class="submit">
          <button onclick="submitFormDBahan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
            <span>Simpan</span>
          </button>
        </li>
        @endif
...
<script>
  function submitFormDBahan_{{$elemen_id}}(){
    var form = $("#formDBahan_{{ $elemen_id }}");
    $.ajax({
