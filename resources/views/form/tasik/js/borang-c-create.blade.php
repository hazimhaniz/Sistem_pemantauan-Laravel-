<script type="text/javascript">
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenTasikBorangCDatatable').DataTable().ajax.reload();
                                    });
                                }
                            },
                            fail: (response) => {
    ...
    function addGambartasik() {
        var count = $('#gambar_count_tasik').val();
        count = ++count;

        $('#gambar_count_tasik').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambartasik(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambartasik(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporantasik() {
        var count = $('#laporan_count_tasik').val();
        count = ++count;

        $('#laporan_count_tasik').val(count);

        $('#addLaporantasik').append('<tr id="addLaporantasik' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporantasik(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporantasik(argument) {
        $('#addLaporantasik' + argument).remove();
    }
</script>