...
<script type="text/javascript">
    ...
    function displayDateEIA() {
        var udara_is_eia = document.getElementById("udara_is_eia");
        var show_date_eia = document.getElementById("show_date_eia")
        if (udara_is_eia.checked) {
    ...
    $(`#udara_date_eia`).datepicker({
        format: `dd/mm/yyyy`
    });
    $(`#udara_date_emp`).datepicker({
        format: `dd/mm/yyyy`
    });
                ...
                $.get("{{ route('form.berfasa') }}", data, (response) => {
                    ...
                    $.each(response.data, (key, value) => {
                        ...
                        if (udara_is_emp.checked) {
                         ...
                         html += '<td class="align-middle text-center"><input class="form-control base_emp_udara" name="base_emp[]" style="display:'+style+'" id="base' + value.id + '" autocomplete="off" onkeypress="return verifyKey(event)"/></td>';
                        html += '</tr>';
                    });

                    $(`#parameterStesenUdaraTable > tbody`).empty().append(html);
                })
        //         .then(() => {
        //             Swal.fire({
        //                 icon: 'success',
    ...
    btnTambahStesenUdara = (elem) => {
        confirmCreate(elem).then((result) => {
            if (result.value) {
                let is_eia = 0;
                let is_emp = 0;

                if ($('#udara_is_eia').is(':checked')) {
                    is_eia = 1;
                }

                if ($('#udara_is_emp').is(':checked')) {
                    is_emp = 1;
                }

                let formData = new FormData();
                formData.append("jenis_pengawasan_id", 6);
                ...
                formData.append("date_eia", $('#udara_date_eia').val());
                formData.append("date_emp", $('#udara_date_emp').val());
                ...
                $('input[name="parameters[]"]').each(function(key, value) {
                    i++;
                    if (i <= "{{ count($pengawasanUdara->parameters) }}") {
                ...
                if (udara_is_emp.checked) {
                    $('input[name="base_emp[]"]').each(function(key, value) {
                        j++;
                        if (j <= "{{ count($pengawasanUdara->parameters) }}") {
