<script type="text/javascript">
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenUdaraDatatable').DataTable().ajax.reload();
                                        $('#stesenUdaraBorangCDatatable').DataTable().ajax.reload();
                                    });
                                }
                            },
                            fail: (response) => {
    ...
    function addGambarudara() {
        var count = $('#gambar_count_udara').val();
        count = ++count;

        $('#gambar_count_udara').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambarudara(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambarudara(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporanudara() {
        var count = $('#laporan_count_udara').val();
        count = ++count;

        $('#laporan_count_udara').val(count);

        $('#addLaporanudara').append('<tr id="addLaporanudara' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporanudara(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporanudara(argument) {
        $('#addLaporanudara' + argument).remove();
    }
</script>