<script type="text/javascript">
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenBunyiBisingBorangCDatatable').DataTable().ajax.reload();
                                    });
                                }
                            },
                            fail: (response) => {
    ...
    function addGambarbunyi() {
        var count = $('#gambar_count_bunyi').val();
        count = ++count;

        $('#gambar_count_bunyi').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambarbunyi(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambarbunyi(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporanbunyi() {
        var count = $('#laporan_count_bunyi').val();
        count = ++count;

        $('#laporan_count_bunyi').val(count);

        $('#addLaporanbunyi').append('<tr id="addLaporanbunyi' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporanbunyi(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporanbunyi(argument) {
        $('#addLaporanbunyi' + argument).remove();
    }
