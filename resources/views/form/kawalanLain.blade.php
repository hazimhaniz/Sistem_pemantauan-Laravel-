<form id="formDLain_{{ $elemen_id }}" method="POST" action="{{ url('/projek/submit-form-d') }}">
    @csrf
            ...
            <table class="table-responsive" id="table" role="grid"  border="1px" style="padding:0px; font-size:12.5px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <div tabindex="500">
                            <textarea name="ulasan[]" id="ulasan_{{ $elemen_id }}_{{ $MasterBMP->id }}" class="form-control border border-default rounded" style="height: 35px;" {{($borangD->status_id==602 || $borangD->status_id==13)?'disabled':''}} aria-invalid="false"></textarea>
                        </div>
                        <br>
                        <div tabindex="500">
                            ...
                            <div id="downloadFile_{{ $elemen_id }}_{{ $MasterBMP->id }}" class="downloadFileDivLain">
...
<div class="col-md-12 p-t-20">
    <ul class="pager wizard no-style">
        ...
        @if($borangD->status_id == 600)
        <li>
            <button id="tindakanBorangDEO" onclick="tindakanBorangD({{ $borangD->id }}, 13)" type="button" class="btn btn-success btn-cons from-left pull-right">
                <span>Hantar</span>
            </button>
        </li>
        @endif
        ...
        @if($borangD->status_id == 600)
        <li class="submit">
            <button onclick="submitFormDLain_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
                <span>Simpan</span>
            </button>
        </li>
        @endif
...
<script>
    function submitFormDLain_{{ $elemen_id }}()
    {
        var form = $("#formDLain_{{ $elemen_id }}");
