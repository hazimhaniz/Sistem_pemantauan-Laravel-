<script type="text/javascript">
    let data = {
        jenis_pengawasan_id: 1
    };

    $('#stesenSungaiDatatable').DataTable({
        'serverSide': true,
        'processing': true,
        'ordering': false,
        'searching': false,
        ...
        columnDefs: [
            ...
            "render": function(id, type, full, meta) {
                ...
                let viewUrl = `{{ route('project.view.stesen.sungai.modal', 'data-id') }}`;
                let editUrl = `{{ route('project.edit.stesen.sungai.modal', 'data-id') }}`;
                let deleteUrl = `{{ route('project.delete.stesen.sungai.modal', 'data-id') }}`;
                ...
                if("{{auth()->user()->entity_type }}" == 'App\UserEMC' && (full.status == 607)) {
                 ...
                 if("{{ auth()->user()->hasRole('penyiasat') }}" && (full.status == 4)) {
                    html += `<button data-action="${viewUrl}" class="btn btn-default btn-xs" type="button" onclick="getModalContent(this)" data-original-title="Sahkan"><span style="color:#000; font-family: 'Montserrat'; font-size: 10.5px; text-transform: uppercase; font-weight: 500;">
    ...
    $('#stesenSungaiBorangCDatatable').DataTable({
        'serverSide': true,
        'processing': true,
        'ordering': false,
        'searching': false,
        ...
        columnDefs: [
            ...
            "render": function(id, type, full, meta) {
                ...
                let viewUrl = `{{ route('form.sungai.borang-c-view', 'data-id') }}`;
                // let deleteUrl = `{{ route('project.delete.borangc.sungai.modal', 'data-id') }}`;
                let editUrl = `{{ route('form.sungai.borang-c-create', 'data-id') }}`;
