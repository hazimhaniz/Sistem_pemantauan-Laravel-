<script type="text/javascript">
    ...
    btnTambahBorangC = (elem) => {
        confirmCreate(elem).then((result) => {
            if (result.value) {
                ...
                formData.append("borangcid", $("#borangc_id").val());
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenSungaiBorangCDatatable').DataTable().ajax.reload();
                                        location.reload();
                                    });
                                }
                            },
    ...
    function addGambarsungai() {
    var count = $('#gambar_count_sungai').val();
    count = ++count;
 
   $('#gambar_count_sungai').val(count);

    $('#addGambar').append('<tr id="addGambar'+count+'"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambarsungai('+count+')" style="font-size: 12.5px;">-</button ></td></tr>')
    }
    function removeGambarsungai(argument){
        $('#addGambar'+argument).remove();
    }
    function addLaporansungai() {
    var count = $('#laporan_count_sungai').val();
    count = ++count;
 
   $('#laporan_count_sungai').val(count);

    $('#addLaporansungai').append('<tr id="addLaporansungai'+count+'"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporansungai('+count+')" style="font-size: 12.5px;">-</button ></td></tr>')
    }
    function removeLaporansungai(argument){
        $('#addLaporansungai'+argument).remove();
    }
</script>