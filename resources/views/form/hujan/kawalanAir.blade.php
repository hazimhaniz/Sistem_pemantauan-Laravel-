<form id="formDAirHujan_{{ $elemen_id }}" method="POST" action="{{ url('/projek/submit-form-d') }}">
  @csrf
        ...
        <tr>
          ...
          <td bgcolor="#f0f0f0" class="text-center">
           <div tabindex="500">
            ...
            <div id="downloadFile_hujan_{{ $elemen_id }}_{{ $MasterBMP->id }}" class="downloadFileHujanDivAir">
...
<div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
      <ul class="pager wizard no-style">
        ...
        @if($laporan_hujan->status_id == 600)
        <li class="submit">
          <button onclick="submitFormDAirHujan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
            <span>Simpan</span>
          </button>
        </li>
        @endif
...
<script>
  function submitFormDAirHujan_{{ $elemen_id }}()
  {
    var form = $("#formDAirHujan_{{ $elemen_id }}");
...
<script>
 ...
 $(document).ready(function(){
   $(".downloadFileHujanDivAir").hide();

   populateBorangHujan = function(){
    $.ajax({
      url: "{{ route('projek.getHujan',$laporan_hujan->id) }}",
      method: "GET",
      dataType: 'json',
      async: true,
      contentType: false,
