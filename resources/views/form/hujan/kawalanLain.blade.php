<form id="formDLainHujan_{{ $elemen_id }}" method="POST" action="{{ route('projek.submitHujan') }}">
  @csrf
      ...
      <tr>
        ...
        <td bgcolor="#f0f0f0" class="text-center">
         <div tabindex="500">
          ...
          <div id="downloadFile_hujan_{{ $elemen_id }}_{{ $MasterBMP->id}}" class="downloadFileHujanDivLain">
...
<div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
    <ul class="pager wizard no-style">
     ...
     @if($laporan_hujan->status_id == 600)
     <li class="submit">
      <button onclick="submitFormDLainHujan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
        <span>Simpan</span>
      </button>
    </li>
    @endif
...
<script>
  function submitFormDLainHujan_{{ $elemen_id }}()
  {
    var form = $("#formDLainHujan_{{ $elemen_id }}");
