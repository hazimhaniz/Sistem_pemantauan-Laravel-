<form id="formDPlaceHujan_{{ $elemen_id }}" method="POST" action="{{ route('form.laporanHujan') }}">
   @csrf
   <div class="row">
    @if($laporan_hujan)
    ...
    <div class="col-md-12">
      <input id="tarikh_hujan_copy2" type="hidden" readonly name="tarikh_hujan" class="form-control" required>
      <input id="tempoh_hujan_copy2" type="hidden" readonly name="tempoh_hujan" min="1" class="form-control" value="1" required>
      <input id="bacaan_hujan_copy2" type="hidden" readonly name="bacaan_hujan" min="12.5" class="form-control" value="12.5" required>
         ...
         <tr>
          ...
          <td bgcolor="#f0f0f0" class="align-center text-center">
            <div tabindex="500">
              ...
              <div id="downloadFile_hujan_{{ $elemen_id }}_{{ $pematuhan->id }}" style="border: none !important; b">
              </div>
              @endif
            </div>
          </td>
  ...
  <div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
      <ul class="pager wizard no-style">
        ...
        @if($laporan_hujan->status_id == 600)
        <li class="submit">
          <button onclick="submitFormDPlaceHujan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
            <span>Simpan</span>
          </button>
        </li>
        @endif
...
<script>
  $(document).ready(function() { 
    $('#tarikh_hujan').change(function() {
      $('#tarikh_hujan_copy2').val($('#tarikh_hujan').val());
    });
    $('#tempoh_hujan').change(function() {
      $('#tempoh_hujan_copy2').val($('#tempoh_hujan').val());
    });
    $('#bacaan_hujan').change(function() {
      $('#bacaan_hujan_copy2').val($('#bacaan_hujan').val());
    });
  });

  function submitFormDPlaceHujan_{{$elemen_id}}(){
    var form = $("#formDPlaceHujan_{{ $elemen_id }}");
    $.ajax({
           ...
           if (monthlyD.doc_type.length > 0) {
             ...
             for (let a = 0; a < monthlyD.doc_type.length; a++) {
              if (monthlyD.doc_type[a].path.length > 0) {
                ...
                $(`#downloadFile_hujan_{{$elemen_id}}_` + monthlyD.elemen_pemeriksaan).html(html);
              }
            }
          }else{
