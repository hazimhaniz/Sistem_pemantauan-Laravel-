<form id="formDEAirHujan_{{ $elemen_id }}" method="POST" action="{{ route('form.laporanHujan') }}">
  @csrf
  <div class="row">
    @if($laporan_hujan)
  ...
  <?php $no=0; $no2=0; ?>
  <div class="col-md-12">
    <input id="tarikh_hujan_copy3" type="hidden" readonly name="tarikh_hujan" class="form-control" required>
    <input id="tempoh_hujan_copy3" type="hidden" readonly name="tempoh_hujan" min="1" class="form-control" value="1" required>
    <input id="bacaan_hujan_copy3" type="hidden" readonly name="bacaan_hujan" min="12.5" class="form-control" value="12.5" required>
    ...
    <table class="" id="table" role="grid" aria-describedby="table_info" border="1px" style="padding:0px;">
      ...
      <tbody>
        ...
        <tr>
          <td bgcolor="#EFD9DB" rowspan="6">{{$MasterBMP->komponen}}</td>
        </tr>
  ...
  <div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
      <ul class="pager wizard no-style">
       ...
       @if($laporan_hujan->status_id == 600)
       <li class="submit">
        <button onclick="submitFormDEAirHujan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
          <span>Simpan</span>
        </button>
      </li>
      @endif
...
<script>
  $(document).ready(function() { 
    $('#tarikh_hujan').change(function() {
      $('#tarikh_hujan_copy3').val($('#tarikh_hujan').val());
    });
    $('#tempoh_hujan').change(function() {
      $('#tempoh_hujan_copy3').val($('#tempoh_hujan').val());
    });
    $('#bacaan_hujan').change(function() {
      $('#bacaan_hujan_copy3').val($('#bacaan_hujan').val());
    });
  });

  function submitFormDEAirHujan_{{$elemen_id}}(){
    var form = $("#formDEAirHujan_{{ $elemen_id }}");
    $.ajax({
