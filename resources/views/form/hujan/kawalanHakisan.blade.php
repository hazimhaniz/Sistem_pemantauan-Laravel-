<form id="formDHakisanHujan_{{ $elemen_id }}" method="POST" action="{{ route('projek.submitHujan') }}">
    @csrf
            ...
            <tr>
                ...
                <td bgcolor="#f0f0f0" class="text-center">
                   <div tabindex="500">
                      ...
                      <div id="downloadFile_hujan_{{ $elemen_id }}_{{ $MasterBMP->id }}" class="downloadFileHujanDivHakisan">
...
<div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
    <ul class="pager wizard no-style">
        ...
        @if($laporan_hujan->status_id == 600)
        <li class="submit">
            <button onclick="submitFormDHakisanHujan_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
                <span>Simpan</span>
            </button>
        </li>
        @endif
...
<script>
  ...
  function submitFormDHakisanHujan_{{ $elemen_id }}()
  {
    var form = $("#formDHakisanHujan_{{ $elemen_id }}");
...
<script>
    ...
    $(document).ready(function(){
     $(".downloadFileHujanDivHakisan").hide();
                ...
                if (hujan.doc_type.length > 0) {
                 ...
                 for (let a = 0; a < hujan.doc_type.length; a++) {
                  if (hujan.doc_type[a].path.length > 0) {
                   ...
                   $('.downloadFileHujanDivHakisan').show();
                    }
                }
            }else{
