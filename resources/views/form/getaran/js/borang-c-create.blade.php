<script type="text/javascript">
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        ...
                                        $('#stesenGetaranBorangCDatatable').DataTable().ajax.reload();
                                    });
                                }
                            },
                            fail: (response) => {
    ...
    function addGambargetaran() {
        var count = $('#gambar_count_getaran').val();
        count = ++count;

        $('#gambar_count_getaran').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambargetaran(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambargetaran(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporangetaran() {
        var count = $('#laporan_count_getaran').val();
        count = ++count;

        $('#laporan_count_getaran').val(count);

        $('#addLaporangetaran').append('<tr id="addLaporangetaran' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporangetaran(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporangetaran(argument) {
        $('#addLaporangetaran' + argument).remove();
    }
</script>