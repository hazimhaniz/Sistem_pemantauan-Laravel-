...
<script type="text/javascript">
    ...
    function displayDateEIA() {
        var airtanah_is_eia = document.getElementById("airtanah_is_eia");
        var show_date_eia = document.getElementById("show_date_eia")

        if (airtanah_is_eia.checked) {
    ...
    $(`#airtanah_date_eia`).datepicker({
        format: `dd/mm/yyyy`
    });
    $(`#airtanah_date_emp`).datepicker({
        format: `dd/mm/yyyy`
    });
                ...
                $.get("{{ route('form.berfasa') }}", data, (response) => {
                    ...
                    $.each(response.data, (key, value) => {
                        ...
                        if (airtanah_is_emp.checked) {
                         ...
                         html += '<td class="align-middle text-center base_emp_air" style="display:'+style+'"><input class="form-control base_emp_air" name="base_emp[]" style="display:'+style+'" id="base' + value.id + '" autocomplete="off" onkeypress="return verifyKey(event)"/></td>';

                        html += '</tr>';
                    });

                    $(`#parameterStesenAirTanahTable > tbody`).empty().append(html);
                })
        //         .then(() => {
        //             Swal.fire({
        //                 icon: 'success',
    ...
    btnTambahStesenAirTanah = (elem) => {
        confirmCreate(elem).then((result) => {
            if (result.value) {
                let is_eia = 0;
                let is_emp = 0;

                if ($('#airtanah_is_eia').is(':checked')) {
                    is_eia = 1;
                }

                if ($('#airtanah_is_emp').is(':checked')) {
                    is_emp = 1;
                }

                let formData = new FormData();
                formData.append("jenis_pengawasan_id", 4);
                ...
                formData.append("date_eia", $('#airtanah_date_eia').val());
                formData.append("date_emp", $('#airtanah_date_emp').val());
                ...
                $('input[name="parameters[]"]').each(function(key, value) {
                    i++;
                    if (i <= "{{ count($pengawasanAirTanah->parameters) }}") {
                ...
                if (airtanah_is_emp.checked) {
                    $('input[name="base_emp[]"]').each(function(key, value) {
                        j++;
                        if (j <= "{{ count($pengawasanAirTanah->parameters) }}") {
