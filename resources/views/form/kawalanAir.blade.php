<form id="formDAir_{{ $elemen_id }}" method="POST" action="{{ url('/projek/submit-form-d') }}">
    @csrf
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td bgcolor="#f0f0f0">
                            ...
                            <div class="form-group alignment text-left">
                                <div class="radio radio-primary">
                                    <input type="radio" value="1" name="status_{{ $MasterBMP->id }}" id="status_{{ $elemen_id }}_{{ $MasterBMP->id }}_1" {{($borangD->status_id==602 || $borangD->status_id==13)?'disabled':''}} required >
                                    <label for="status_{{ $elemen_id }}_{{ $MasterBMP->id }}_1">Baik</label>
                                </div>

                                <div class="radio radio-primary">
                                    <input type="radio" value="2" name="status_{{ $MasterBMP->id }}" id="status_{{ $elemen_id }}_{{ $MasterBMP->id }}_2" {{($borangD->status_id==602 || $borangD->status_id==13)?'disabled':''}} required>
                                    <label for="status_{{ $elemen_id }}_{{ $MasterBMP->id }}_2">Perlu diselenggara</label>
                                </div>

                                <div class="radio radio-primary">
                                    <input type="radio" value="3" name="status_{{ $MasterBMP->id }}" id="status_{{ $elemen_id }}_{{ $MasterBMP->id }}_3"  {{($borangD->status_id==602 || $borangD->status_id==13)?'disabled':''}} required>
                        ...
                        <div tabindex="500">
                            ...
                            <div id="downloadFile_{{ $elemen_id }}_{{ $MasterBMP->id }}" class="downloadFileDivAir">
...
<div class="col-md-12 p-t-20" style="padding-bottom: 15px;">
    <ul class="pager wizard no-style">
        <input type="hidden" name="flag" id="flagD2" value="">
        @if($borangD)
        @hasanyrole('eo')
        @if($borangD->status_id == 600)
        <li class="submit">
            <button onclick="submitFormDAir_{{ $elemen_id }}()" class="btn btn-info btn-cons from-left pull-right" name="submit" value="Simpan" type="button">
                <span>Simpan</span>
            </button>
        </li>
        @endif
...
<script>
    function submitFormDAir_{{ $elemen_id}}() {
        var form = $("#formDAir_{{ $elemen_id }}");
