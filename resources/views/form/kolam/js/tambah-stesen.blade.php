...
<script type="text/javascript">
    ...
    function displayDateEIA() {
        var kolam_is_eia = document.getElementById("kolam_is_eia");
        var show_date_eia = document.getElementById("show_date_eia")
        if (kolam_is_eia.checked) {
    ...
    $(`#kolam_date_eia`).datepicker({
        format: `dd/mm/yyyy`
    });
    $(`#kolam_date_emp`).datepicker({
        format: `dd/mm/yyyy`
    });
                ...
                $.get("{{ route('form.berfasa') }}", data, (response) => {
                    ...
                    $.each(response.data, (key, value) => {
                        ...
                        if (kolam_is_emp.checked) {
                         ...
                         html += '<td class="align-middle text-center base_emp_kolam" style="display:'+style+'"><input class="form-control base_emp_kolam" name="base_emp[]" style="display:'+style+'" id="base' + value.id + '" autocomplete="off" onkeypress="return verifyKey(event)"/></td>';
                        html += '</tr>';
                    });

                    $(`#parameterStesenKolamTable > tbody`).empty().append(html);
                })
        //         .then(() => {
        //             Swal.fire({
        //                 icon: 'success',
    ...
    btnTambahStesenKolam = (elem) => {
        confirmCreate(elem).then((result) => {
            if (result.value) {
                let is_eia = 0;
                let is_emp = 0;

                if ($('#kolam_is_eia').is(':checked')) {
                    is_eia = 1;
                }

                if ($('#kolam_is_emp').is(':checked')) {
                    is_emp = 1;
                }

                let formData = new FormData();
                formData.append("jenis_pengawasan_id", 5);
                ...
                formData.append("date_eia", $('#kolam_date_eia').val());
                formData.append("date_emp", $('#kolam_date_emp').val());
                ...
                $('input[name="parameters[]"]').each(function(key, value) {
                    i++;
                    if (i <= "{{ count($pengawasanKolam->parameters) }}") {
                ...
                if (kolam_is_emp.checked) {
                    $('input[name="base_emp[]"]').each(function(key, value) {
                        j++;
                        if (j <= "{{ count($pengawasanKolam->parameters) }}") {
