<script type="text/javascript">
    ...
    function addGambarkolam() {
        var count = $('#gambar_count_kolam').val();
        count = ++count;

        $('#gambar_count_kolam').val(count);

        $('#addGambar').append('<tr id="addGambar' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="gambar_pengsampelans" name="gambar_pengsampelans[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeGambarkolam(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeGambarkolam(argument) {
        $('#addGambar' + argument).remove();
    }

    function addLaporankolam() {
        var count = $('#laporan_count_kolam').val();
        count = ++count;

        $('#laporan_count_kolam').val(count);

        $('#addLaporankolam').append('<tr id="addLaporankolam' + count + '"><td><div tabindex="500" class=""><i class="fa fa-folder-open"></i> <input class="laporan_kimias" name="laporan_kimias[]" type="file"></div></td><td><button type="button" class="btn btn-danger btn-xs pull-right" onclick="removeLaporankolam(' + count + ')" style="font-size: 12.5px;">-</button ></td></tr>')
    }

    function removeLaporankolam(argument) {
        $('#addLaporankolam' + argument).remove();
    }
</script>