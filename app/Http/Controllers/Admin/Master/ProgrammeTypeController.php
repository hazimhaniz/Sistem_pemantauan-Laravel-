...
class ProgrammeTypeController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if($request->ajax()) {
            ...
            $log->description = "Papar senarai Data Induk - Jenis Program";
            ...
            $type = MasterProgrammeType::orderBy('id');
            ...
            $log->description = "Buka paparan Data Induk - Jenis Program";
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
            $log->created_by_user_id = auth()->id();
        ...
        return view('admin.master.programme-type.index');
    }
    ...
    public function insert(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|unique:master_programme_type',
        ]);
        ...
        $type = MasterProgrammeType::create($request->only('name'));

        $log = new LogSystem;
        $log->module_id = \App\MasterModel\MasterModule::where('code','master_data')->firstOrFail()->id;
        $log->activity_type_id = 4;
        $log->description = "Tambah Data Induk - Jenis Program";
    ...
    public function edit(Request $request)
    {
        ...
        $log->description = "Popup kemaskini Data Induk - Jenis Program";
        $log->url = $request->fullUrl();
        $log->method = strtoupper($request->method());
        $log->ip_address = $request->ip();
        $log->created_by_user_id = auth()->id();
        ...
        $type = MasterProgrammeType::findOrFail($request->id);

        return view('admin.master.programme-type.edit', compact('type'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $log->description = "Kemaskini Data Induk - Jenis Program";
    ...
    public function delete(Request $request)
    {
        ...
        $log->description = "Padam Data Induk - Jenis Program";
        $log->data_old = json_encode($type);
        $log->url = $request->fullUrl();
        $log->method = strtoupper($request->method());
        $log->ip_address = $request->ip();
